"use client";

import { useMemo } from "react";
import StatusGrid from "@/components/StatusGrid";
import CallSheetDisplay from "@/components/CallSheet";
import { useRoya } from "@/components/RoyaProvider";
import { getStatus, getCallSheet, getAvailableDates } from "@/lib/client-store";
import { formatCallSheetText, formatCallSheetHTML } from "@/lib/callsheet-builder";

export default function DashboardPage() {
  const { state, origin } = useRoya();

  const status = useMemo(() => getStatus(), [state.scenes, state.tasks]);
  const availableDates = useMemo(() => getAvailableDates(), [state.scenes]);
  const selectedDate = availableDates.length > 0 ? availableDates[0] : null;
  const callSheet = selectedDate ? getCallSheet(selectedDate) : null;

  function download(filename: string, content: string, type: string) {
    const blob = new Blob([content], { type });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = filename;
    a.click();
    URL.revokeObjectURL(url);
  }

  function exportCallSheet(format: "text" | "html") {
    if (!callSheet) return;
    const filename = `call-sheet-day${callSheet.dayNumber}.${format === "text" ? "txt" : "html"}`;
    const content = format === "text" ? formatCallSheetText(callSheet) : formatCallSheetHTML(callSheet);
    download(filename, content, format === "text" ? "text/plain" : "text/html");
  }

  return (
    <div className="space-y-8">
      <header>
        <h1 className="text-2xl md:text-3xl font-bold text-amber-500 leading-tight">📊 Production Dashboard</h1>
        <p className="text-gray-400 mt-2 text-sm md:text-base">
          Overview of your production status, schedule, and call sheets.
        </p>
        <div className="mt-2 flex items-center gap-2">
          {origin === "demo" && (
            <span className="badge bg-blue-500/20 text-blue-400 border border-blue-500/30">Demo data</span>
          )}
          {origin === "analysis" && (
            <span className="badge bg-amber-500/20 text-amber-400 border border-amber-500/30">Your analysis</span>
          )}
        </div>
      </header>

      {state.scenes.length > 0 && <StatusGrid status={status} />}

      {state.scenes.length > 0 && status.weatherAlerts.length > 0 && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-lg px-5 py-4">
          <h3 className="text-sm font-bold text-red-400 mb-2">⚠️ Weather Alerts</h3>
          {status.weatherAlerts.map((alert, i) => (
            <p key={i} className="text-xs text-red-300">{alert}</p>
          ))}
        </div>
      )}

      {state.scenes.length === 0 ? (
        <div className="card text-center py-12">
          <p className="text-gray-500 text-lg">No production data yet.</p>
          <p className="text-gray-600 text-sm mt-2">
            Analyze a screenplay and generate a schedule to see your dashboard.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="card">
            <h2 className="text-lg font-bold text-gray-100 mb-4">📅 Call Sheet</h2>
            {availableDates.length > 0 ? (
              <div className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  {availableDates.map((d) => (
                    <button
                      key={d}
                      className={`text-sm px-3 py-1.5 rounded-lg transition-all ${
                        selectedDate === d
                          ? "bg-amber-500/20 text-amber-400 border border-amber-500/30"
                          : "text-gray-500 hover:text-gray-300 bg-dark-800 border border-dark-500"
                      }`}
                    >
                      {d}
                    </button>
                  ))}
                </div>
                {callSheet && (
                  <CallSheetDisplay sheet={callSheet} onExport={exportCallSheet} />
                )}
              </div>
            ) : (
              <p className="text-gray-500 text-sm">
                Generate a schedule first to see call sheets.
              </p>
            )}
          </div>

          <div className="card">
            <h2 className="text-lg font-bold text-gray-100 mb-4">🎭 Quick Stats</h2>
            {status && (
              <div className="space-y-4">
                <div className="bg-dark-800 rounded-lg p-4 border border-dark-500">
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-gray-400">Overall Completion</span>
                    <span className="text-amber-400 font-bold">{status.completionPercent}%</span>
                  </div>
                  <div className="w-full bg-dark-600 rounded-full h-2">
                    <div
                      className="bg-amber-500 h-2 rounded-full transition-all duration-500"
                      style={{ width: `${status.completionPercent}%` }}
                    />
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-dark-800 rounded-lg p-3 border border-dark-500 text-center">
                    <p className="text-xl font-bold text-blue-400">{status.totalScenes}</p>
                    <p className="text-xs text-gray-500">Scenes</p>
                  </div>
                  <div className="bg-dark-800 rounded-lg p-3 border border-dark-500 text-center">
                    <p className="text-xl font-bold text-purple-400">{status.totalTasks}</p>
                    <p className="text-xs text-gray-500">Tasks</p>
                  </div>
                  <div className="bg-dark-800 rounded-lg p-3 border border-dark-500 text-center">
                    <p className="text-xl font-bold text-green-400">{status.completedScenes}</p>
                    <p className="text-xs text-gray-500">Scenes Done</p>
                  </div>
                  <div className="bg-dark-800 rounded-lg p-3 border border-dark-500 text-center">
                    <p className="text-xl font-bold text-amber-400">{status.completedTasks}</p>
                    <p className="text-xs text-gray-500">Tasks Done</p>
                  </div>
                </div>
              </div>
            )}
          </div>
        </div>
      )}
    </div>
  );
}
